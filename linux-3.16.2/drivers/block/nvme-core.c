						...
						u64 *result, unsigned timeout)
{
	...
	printk(KERN_ALERT "%s: submitting object io request in queue %d\n", __func__, q_idx);
...
static int nvme_submit_object_io(struct nvme_ns *ns, struct nvme_object_io __user *uio)
{
	...
	printk(KERN_ALERT "%s: object_id is %llu\n", __func__, io.object_id);
	printk(KERN_ALERT "%s: object_size is %u\n", __func__, io.size);
	printk(KERN_ALERT "%s: opcode is %u\n", __func__, io.opcode);
	printk(KERN_ALERT "%s: metadata length is %u\n", __func__, meta_len);
	...
	case nvme_cmd_obj_read:
		...
		break;
	default:
		printk(KERN_ALERT "%s: unknown opcode %u\n", __func__, io.opcode);
		return -EINVAL;
	}
